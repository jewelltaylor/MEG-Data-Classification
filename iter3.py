...
ENSEMBLE_SAMPLE_SIZE = .7 #sample size for ensemble bootstrapping
ENSEMBLE_SIZE = 5
...
def generate_model(df):
    ...
    #Bootstrap loop gets new sample data
    for bootstrap_iteration in range(BOOTSTRAP_ITERATIONS):
        ...
        for train_index, test_index in kf.split(correct_X):
            ...
            class_size = correct_train_X.shape[0]
            sample_size = int(class_size * ENSEMBLE_SAMPLE_SIZE)
            model_predictions = []
            for model in ENEMBLE_SIZE:
                correct_train_X, correct_train_y = shuffle(correct_train_X, correct_train_y)
                incorrect_train_X, incorrect_train_y = shuffle(correct_train_X, correct_train_y)
                correct_train_X = correct_train_X[:sample_size]
                correct_train_y = correct_train_y[:sample_size]
                incorrect_train_X = incorrect_train_X[:sample_size]
                incorrect_train_y = incorrect_train_y[:sample_size]
            ...
            ensemble_prediction = mode(model_prediction)

            accuracy = accuracy_score(test_y, ensemble_prediction)
            accuracy_list.append(accuracy)
